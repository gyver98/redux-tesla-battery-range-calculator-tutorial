import React from 'react';
import './TeslaCounter.css';

const SpeedTeslaCounter = ({ initValues, currentValue, speedIncrement, speedDecrement }) => (
  <div className="tesla-counter">
    <p className="tesla-counter__title">{initValues.title}</p>
    <div className="tesla-counter__container cf">
      <div className="tesla-counter__item">
        <p className="tesla-counter__number">
          { currentValue }
          <span>{ initValues.unit }</span>
        </p>
        <div className="tesla-counter__controls">
          <button 
            onClick={(e) => {
              e.preventDefault();
              speedIncrement(currentValue)}} 
            disabled={currentValue >= initValues.max} 
          >
          </button>
          <button 
            onClick={(e) => { 
              e.preventDefault();
              speedDecrement(currentValue)}} 
            disabled={currentValue <= initValues.min} 
          >
          </button>
        </div>
      </div>
    </div>
  </div>  
);

SpeedTeslaCounter.propTypes = {
  currentValue: React.PropTypes.number,
  speedIncrement: React.PropTypes.func,
  speedDecrement: React.PropTypes.func,
  initValues: React.PropTypes.object
}

export default SpeedTeslaCounter;